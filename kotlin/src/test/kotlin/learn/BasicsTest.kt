...
class BasicsTest {
    ...
    fun testClasses() {
        ...
        val withNameRef = fc1::withName
        println("with name of fc1: ${withNameRef("sampleVal")}")
...
//    println("v2=$v2") //runtime exception here - not initialized yet
    ...
    fun testNullable() {
        ...
        nullableV1?.let {
            println("this block is skipped because nullableV1 is null")
        }
    }
...
class FooClass(name: String = defaultName, description: String, count: Int) {
    val name: String
...
//        return "FooClass(name=$name, description=$description)" //doesn't compile - constructor arguments available in constructor only
    ...
    fun withName(value: String): String {
        return "name=$name, value=$value"
    }

    companion object {
        val defaultName: String = "defaultFoo"
    }
}
